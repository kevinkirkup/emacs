...
;; Not ready.
(defun angrytuna-markdown-insert-link ()
...
(add-hook
 'markdown-mode-hook
 '(lambda ()
    (define-key markdown-mode-map (kbd "C-c i") 'angrytuna-markdown-insert-link)
    (define-key markdown-mode-map (kbd "A-r") 'markdown-preview)
    (define-key markdown-mode-map (kbd "<tab>") 'angrytuna-indent)))
